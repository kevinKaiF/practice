...
public class IpSeeker {
    ...
    static {
        ...
        INSTALL_DIR = IP_FILE.replace("qqwry.dat", "");
    }
    ...
    public IpSeeker() {
        ...
        b3 = new byte[3];
        try {
            ipFile = new RandomAccessFile(new File("qqwry.dat"), "r");
        } catch (Exception e) {
        }
        // 如果打开文件成功，读取文件头信息
        if (ipFile != null) {
    ...
    public IpSeeker(String fileName) {
     ...
     * @return IPEntry
     * @throws
     * @Title: getCountryCity
     * @Description: TODO
     */
    public Set<String>[] getAllCountryCity() {
        String arr[] = null;
        ...
        try {
            ...
            for (int offset = (int) ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
                ...
                if (temp != -1) {
                    IPLocation ipLoc = getIPLocation(temp);
                    arr = AdressUtils.parseCountryCity(ipLoc.getCountry());
                    country.add(arr[0]);
                    province.add(arr[1]);
                    city.add(arr[2]);
                }
        ...
        Set sets[] = {country, province, city};
        return sets;
    }
