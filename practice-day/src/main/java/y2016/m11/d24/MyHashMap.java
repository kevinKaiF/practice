...
public class MyHashMap<K,V>
     ...
     */
    transient MyHashMap.Entry<K,V>[] table = (MyHashMap.Entry<K,V>[]) EMPTY_TABLE;
     ...
     * after MyHashMap has been initialized but before any entries have
...
//        }
    ...
    public V get(Object key) {
        ...
        MyHashMap.Entry<K,V> entry = getEntry(key);
    ...
    private V getForNullKey() {
        ...
        for (MyHashMap.Entry<K,V> e = table[0]; e != null; e = e.next) {
            if (e.key == null)
                return e.value;
        }
        return null;
     ...
     * for the key.
     */
    final MyHashMap.Entry<K,V> getEntry(Object key) {
        if (size == 0) {
            return null;
        }

        int hash = (key == null) ? 0 : hash(key);
        for (MyHashMap.Entry<K,V> e = table[indexFor(hash, table.length)];
             e != null;
             e = e.next) {
            Object k;
            if (e.hash == hash &&
    ...
    void resize(int newCapacity) {
        MyHashMap.Entry[] oldTable = table;
        ...
        MyHashMap.Entry[] newTable = new MyHashMap.Entry[newCapacity];
    ...
    void transfer(MyHashMap.Entry[] newTable, boolean rehash) {
        ...
        for (MyHashMap.Entry<K,V> e : table) {
            while(null != e) {
                MyHashMap.Entry<K,V> next = e.next;
                if (rehash) {
    ...
    public V remove(Object key) {
        MyHashMap.Entry<K,V> e = removeEntryForKey(key);
     ...
     * for this key.
     */
    final MyHashMap.Entry<K,V> removeEntryForKey(Object key) {
        if (size == 0) {
            return null;
        }
        int hash = (key == null) ? 0 : hash(key);
        int i = indexFor(hash, table.length);
        MyHashMap.Entry<K,V> prev = table[i];
        MyHashMap.Entry<K,V> e = prev;
     ...
     * for matching.
     */
    final MyHashMap.Entry<K,V> removeMapping(Object o) {
    ...
    public boolean containsValue(Object value) {
        ...
        MyHashMap.Entry[] tab = table;
        for (int i = 0; i < tab.length ; i++)
            for (MyHashMap.Entry e = tab[i]; e != null ; e = e.next)
    ...
    void createEntry(int hash, K key, V value, int bucketIndex) {
        MyHashMap.Entry<K,V> e = table[bucketIndex];
        table[bucketIndex] = new MyHashMap.Entry<>(hash, key, value, e);
        size++;
    }
            ...
            throws IOException, ClassNotFoundException
    {
        ...
        table = (MyHashMap.Entry<K,V>[]) EMPTY_TABLE;
